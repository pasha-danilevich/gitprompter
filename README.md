Вот нормальная README-документация — без воды, но с тем, что реально нужно, чтобы начать пользоваться и понимать, как это работает.

---

# GitPrompter

**GitPrompter** — это CLI-инструмент, который превращает `git diff` и историю коммитов в готовые промпты для генерации commit-сообщений через LLM (например, ChatGPT).
...
| Параметр         | Описание                                           |
| ---------------- | -------------------------------------------------- |
| `style`          | Стиль commit-сообщения (`conventional`, `feature`) |
...
* Требует установленный `pyperclip` для буфера обмена

---

## 🧱 Ограничения

* Нет кастомных шаблонов (style = custom пока не реализован)
* Не анализирует смысл кода — только diff/log
...
* Когда ты ленишься писать commit message (и правильно делаешь 😄)
...
* улучшить UX CLI (например, preview + confirm)
* добавить кастомные шаблоны
* или встроить это прямо в git hooks (очень полезная штука)
